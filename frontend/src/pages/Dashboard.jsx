...
const StatCard = ({ title, value, icon: Icon, colorClass, borderClass, bgClass }) => (
  <div className={`bg-white border ${borderClass} p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow relative overflow-hidden`}>
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const { stats: s, recent_transactions, chart_data } = data;

        setStats({
          totalOmzet: s.total_omzet,
  ...
  if (user?.role === 'kasir') {
      return (
        <div className="h-full flex items-center justify-center p-6 bg-slate-50">
            <div className="text-center bg-white border border-slate-100 p-12 rounded-2xl shadow-md max-w-lg w-full relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="w-24 h-24 bg-slate-100 text-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm rotate-3 hover:rotate-0 transition-transform">
                        <ShoppingBag size={48} />
                    </div>
                    ...
                    <a href="/pos" className="inline-block bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-10 rounded-2xl shadow-sm transition-all active:scale-95 text-lg">
                        Buka Mesin Kasir
                    </a>
                </div>
            </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 bg-slate-50 min-h-full p-2">
      <div className="mb-2">
        ...
        <p className="text-slate-500 font-medium mt-1">Selamat datang <span className="font-bold text-blue-600">{user?.name}</span>! Berikut kilasan laporan toko hari ini.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-slate-100 rounded-2xl p-8 min-h-[400px] flex flex-col justify-center items-center text-slate-400 shadow-sm relative overflow-hidden">
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 min-h-[400px] shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-6">
             <h3 className="text-lg font-black text-slate-800">Riwayat Terakhir</h3>
             <span className="text-xs font-bold text-blue-600 bg-slate-50 px-3 py-1 rounded-full">Baru Saja</span>
          </div>
          ...
          <div className="space-y-4 flex-1">
            {loading ? (
                ...
                <div key={t.id} className="flex justify-between items-center p-4 bg-slate-50 hover:bg-slate-50 rounded-2xl transition-colors cursor-pointer border border-transparent hover:border-slate-100 group">
                    <div className="flex gap-4 items-center">
                        <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 group-hover:text-blue-500 group-hover:border-slate-200 transition-colors">
                           <FileText size={18} />
                        </div>
                        <div>
          ...
          <button className="w-full mt-4 py-3 text-sm font-bold text-blue-600 bg-slate-50 hover:bg-slate-100 rounded-xl transition-colors">
              Lihat Semua Laporan
          </button>
        </div>
      </div>
