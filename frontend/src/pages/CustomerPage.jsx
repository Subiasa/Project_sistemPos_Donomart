        ...
        <div className="space-y-6">
            <div className="flex justify-between items-end">
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold border border-transparent shadow-md shadow-indigo-600/20 active:scale-95 transition-all text-sm"
                >
                    <Plus size={18} />
                    Tambah Pelanggan
                </button>
                                    ...
                                    <tr key={c.id} className="hover:bg-sky-50/30 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-sky-100 text-sky-600 flex items-center justify-center font-bold shadow-sm">{c.nama[0]?.toUpperCase()}</div>
                                                <div>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2">
                                                <button onClick={() => openEdit(c)} className="p-2 text-sky-600 hover:text-white bg-sky-50 hover:bg-sky-500 border border-sky-100 rounded-lg transition-colors">
                                                    <Edit size={16} />
                                                </button>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-800/40 backdrop-blur-sm">
                    <div className="bg-white border border-slate-200 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea value={formData.alamat} onChange={e => setFormData({...formData, alamat: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 focus:border-sky-400 focus:ring-2 focus:ring-sky-100 focus:bg-white outline-none transition-all font-medium resize-none" rows="3"></textarea>
                            </div>
                            <div className="pt-4 flex gap-3 justify-end border-t border-slate-100">
                                ...
                                <button type="submit" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-md shadow-indigo-600/20 active:scale-95 transition-all">Simpan Data</button>
                            </div>
                        </form>
                    </div>
                </div>
